...
public:
  CartState state;
  CartDirection dir;
  Cart(int row,int col,CartDirection d) : row(row),col(col),
					  dir(d),state(left) {};
  int row,col;
...
bool collision(int r, int c, std::vector<Cart> cts) {
  for(auto &cdx : cts) {
    if ((cdx.row == r) && (cdx.col == c)) return true;
  }
  return false;
}
...
int main(int argc, char *argv[]) {
  ...
  while(std::getline(f,line)) {
	...
	break;
      case 'v':
	board[rdx][cdx] = Cell(true,false,true,false);
	carts.push_back(Cart(rdx,cdx,dd));
	break;
      case '^' :
	board[rdx][cdx] = Cell(true,false,true,false);
	carts.push_back(Cart(rdx,cdx,du));
	break;
      case '>':
	board[rdx][cdx] = Cell(false,true,false,true);
	carts.push_back(Cart(rdx,cdx,dr));
	break;
      case '<':
	board[rdx][cdx] = Cell(false,true,false,true);
	carts.push_back(Cart(rdx,cdx,dl));
	break;
      }
    }
    rdx += 1;
