...
public:
  ...
  int move() {
    if (count==0) {
      ...
      Marble *egtcc = current;
      while ((egtcc->next->next->next->next->next->next->next->next)!=current) {
	egtcc = egtcc->next;
      }
...
int main(int argc, char *argv[]) {
  ...
  while(std::getline(f,line)) {
    ...
    mbls = std::stoi(line.substr(line.find("worth")+5,std::string::npos));
  ...
  std::vector<int> scores(pls, 0);
  Board b = Board();
  int maxscore = 0;
  for(int mdx=0; mdx<mbls; ++mdx) {
    int pid = mdx%pls;
    int delta = b.move();
    int newscore = scores[pid]+delta;
